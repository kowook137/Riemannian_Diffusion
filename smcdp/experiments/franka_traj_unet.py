"""Stage 5 — Ours-UNet GOAL-CONDITIONAL trajectory diffusion on Franka 7-DoF.

Implements Idea_formulation §15.1 killer experiment task spec:
    "Robot이 tool grip, 3D target에 tool-tip 도달.
     Redundant kinematics → 같은 target에 multi-modal trajectory."

Combines every component of the SMCDP framework:
  - LearnedSelfModelFranka7DoF  (analytic FK + frozen Δ_φ from Stage-1)
  - Embodiment context z_e = tool-tip offset along panda_hand z-axis
  - Bimodal IK demos (mirror q_1 swing — analogue of elbow-up/down for 7-DoF)
  - GOAL-CONDITIONAL trajectory: cond = (z_e, p_target ∈ R^3)
  - Trajectory-level Riemannian SGM on M_φ(z_e)^{H+1}
  - DSM-Varadhan loss with corrected τ = ∫₀ᵗ β(s) ds
  - ConditionalUnet1D score net (DP architecture inside our Riemannian framework)

Validates §10 contributions (C1, C2, C3) AND §15.1 task definition jointly.

§15.4 metrics:
  Task performance:        Success rate, tool-tip position error, reach error
  Manifold adherence:      max ‖g_φ(x_t)‖, mean ‖g_φ‖ (= 0 by construction)
  Multi-modal capture:     mode coverage (frac_A), per-mode chart W₁
  Robustness:              vs z_e (in-distribution / OOD)

Run:
    python -m smcdp.experiments.franka_traj_unet \
        --stage1-ckpt outputs/franka_stage1/delta_phi.pt
"""
...
def parse_args():
    ...
    p.add_argument("--stage1-ckpt", type=str,
                   default="outputs/franka_stage1/delta_phi.pt")
    p.add_argument("--H", type=int, default=15)               # H+1=16 (UNet div by 4)
    # demo
    ...
    # eval
    p.add_argument("--n-eval-per-z", type=int, default=128)
    p.add_argument("--n-targets-per-z", type=int, default=8)   # number of distinct p_target draws per z_e
    p.add_argument("--success-radius", type=float, default=0.02)  # m, "target 도달" criterion
    p.add_argument("--z-eval", type=float, nargs="+",
                   default=[0.05, 0.10, 0.15, 0.20])         # last is OOD
    p.add_argument("--seed", type=int, default=0)
    p.add_argument("--out-dir", type=str, default="outputs/franka_traj_unet")
    p.add_argument("--resume-from", type=str, default=None,
                   help="Path to ckpt_*.pt — load net + ema_net + optimizer state and "
                        "continue training for `--steps` MORE steps.")
    # CFG (classifier-free guidance, Ho & Salimans 2022)
    p.add_argument("--cond-drop-prob", type=float, default=0.10,
                   help="Per-sample prob of replacing p_target with null (zeros) during training.")
    p.add_argument("--guidance-scale", type=float, default=0.0,
                   help="CFG sampling weight w; 0 = cond only, >0 amplifies cond signal.")
    # Phase 6.1 — channel-concat conditioning + Phase 5.2 — endpoint loss reweighting
    p.add_argument("--cond-injection", type=str, default="global",
                   choices=["global", "channel"],
                   help="Phase 6.1: 'channel' broadcasts cond across timesteps as input channels.")
    p.add_argument("--endpoint-weight", type=float, default=1.0,
                   help="Phase 5.2: extra weight on h=H timestep loss (1.0 = uniform).")
    # Sampling-time goal residual guidance (Phase 5.3)
    p.add_argument("--goal-residual-alpha", type=float, default=0.0,
                   help="Phase 5.3: explicit Riemannian endpoint-pull at sampling time.")
    p.add_argument("--goal-residual-h-mode", type=str, default="last_half",
                   choices=["last_only", "last_quarter", "last_half", "all"])
    p.add_argument("--start-anchor-alpha", type=float, default=0.0,
                   help="Start anchor analytic guidance at h=0.")
    p.add_argument("--smoothness-alpha-vel", type=float, default=0.0,
                   help="Quadratic velocity penalty in score guidance.")
    p.add_argument("--smoothness-alpha-acc", type=float, default=0.0,
                   help="Quadratic acceleration penalty in score guidance.")
    p.add_argument("--use-p-start-cond", action="store_true",
                   help="Append p_start to goal_cond (model goal_cond_dim becomes 6).")
    return p.parse_args()
...
def main():
    ...
    print(f"loaded Stage-1: improvement={ck['metrics']['improvement_factor']:.1f}x   "
          f"learned mean ‖p_true-F_φ‖={ck['metrics']['err_learned_mean']:.4e}")
    ...
    arm = LearnedSelfModelFranka7DoF(
        ...
        tool_z_max=max(args.z_max, args.z_eval[-1]) + 0.05, metric=args.metric,
    )
    ...
    # ---- score net (goal-conditional: cond = z_e + p_target [+ p_start]) + EMA ----
    ...
    if args.resume_from is not None:
        ...
        net.load_state_dict(resume_ck["net_state"])
        ema_net.load_state_dict(resume_ck["ema_state"])
        if "optim_state" in resume_ck:
            optim.load_state_dict(resume_ck["optim_state"])
            print(f"resumed from {args.resume_from} (with optimizer state)")
        else:
            print(f"resumed from {args.resume_from} (no optimizer state — fresh Adam)")
        # warmup already finished — set LR scheduler to 1.0 throughout
        lr_sched = torch.optim.lr_scheduler.LambdaLR(optim, lr_lambda=lambda s: 1.0)

    # ---- training ----
    losses = []
    pbar = tqdm(range(args.steps), desc=f"train Ours-UNet/{args.metric}")
    for step in pbar:
        x_data, _, _, p_target, p_start = data.sample(args.batch, device=device)
        if args.use_p_start_cond:
            cond_for_net = torch.cat([p_target, p_start], dim=-1)              # (B, 6)
        else:
            cond_for_net = p_target                                             # (B, 3)
        loss = traj_dsm_varadhan_loss(score_fn_train, sde, x_data,
    ...
    # Pre-sample target points and replicate per-target evaluation
    ...
    for i, z_val in enumerate(args.z_eval):
        # Per-z evaluation: sample n_eval_per_z trajectories with random p_target
        # in box (in-distribution targets), conditioned on this z_e.
        torch.manual_seed(args.seed + 1000 + i)
        p_targets = box_lo + (box_hi - box_lo) * torch.rand(n, 3, device=device)

        # Data reference at this (z_e, p_target): one trajectory per (z, target)
        ...
        x_data, branch_A_data, _, _, p_starts = data_z.sample(
            n, device=device, p_target=p_targets,
        )
        if args.use_p_start_cond:
        ...
        # Reverse SDE conditioned on each (z_e, p_target)
        ...
        with torch.no_grad():
            tau_gen = traj_reverse_grw(sde, score_fn_eval, tau_T,
                                       ...
                                       goal_cond=cond_for_eval,
                                       guidance_scale=args.guidance_scale,
                                       goal_residual_alpha=args.goal_residual_alpha,
                                       goal_residual_h=grh,
                                       p_start=p_starts,
                                       start_anchor_alpha=args.start_anchor_alpha,
                                       smoothness_alpha_vel=args.smoothness_alpha_vel,
                                       smoothness_alpha_acc=args.smoothness_alpha_acc)

        # Constraint adherence on LEARNED manifold (must be 0 by lift)
        g_learned = arm.constraint(tau_gen.reshape(-1, d)).norm(dim=-1)

        # §15.4 Task performance
        p_gen_end = tau_gen[:, -1, 7:10]
        p_data_end = x_data[:, -1, 7:10]
        # tool-tip position error (gen end vs target)
        pos_err_gen = (p_gen_end - p_targets).norm(dim=-1)
        pos_err_data = (p_data_end - p_targets).norm(dim=-1)
        # success rate: within success_radius
        success_gen = (pos_err_gen < args.success_radius).float().mean().item()
        success_data = (pos_err_data < args.success_radius).float().mean().item()

        # Mode classification: q_1 sign at midpoint
        ...
        between = (q1_mid_gen.abs() < 0.10).float().mean().item()

        # Per-mode chart W₁ (random direction projection)
        n_dir = 64
        ...
        # Joint limit violations on generated
        violates_gen = arm.violates_limits(tau_gen.reshape(-1, d)[..., :7]).float().mean().item()

        metrics_per_z[z_val] = {
            "max_g_learned": g_learned.max().item(),
            "mean_g_learned": g_learned.mean().item(),
            "pos_err_gen_mean": pos_err_gen.mean().item(),
            "pos_err_gen_med":  pos_err_gen.median().item(),
            "pos_err_data_mean": pos_err_data.mean().item(),
            "success_rate_gen": success_gen,
            "success_rate_data": success_data,
            "frac_A_data": frac_A_data,
            "frac_A_gen": frac_A_gen,
            "frac_between_modes": between,
            "w1_A": w1_A,
            "w1_B": w1_B,
            "violates_gen": violates_gen,
        }
        ood_tag = "  OOD" if (z_val < args.z_min or z_val > args.z_max) else ""
        print(f"  z_e = {z_val:.3f}{ood_tag}:  "
              f"max|g|={g_learned.max():.1e}   "
              f"success gen/data = {success_gen:.3f}/{success_data:.3f}   "
              f"pos_err gen = {pos_err_gen.mean():.4f}   "
              f"frac_A data/gen = {frac_A_data:.3f}/{frac_A_gen:.3f}   "
              f"W₁ A/B = {w1_A:.3f}/{w1_B:.3f}   "
              f"viol={violates_gen*100:.1f}%")
        ...
        ax.set_xlabel("q_1 mid"); ax.set_ylabel("density")
        ax.set_title(f"z_e={z_val:.3f}{ood_tag}  frac_A={frac_A_gen:.2f}")
        ax.legend(fontsize=7); ax.grid(alpha=0.3)

        ax = axes[1, i]
        ax.scatter(p_targets[:, 0].cpu(), p_targets[:, 2].cpu(),
                   alpha=0.5, s=12, label="p_target", color="k", marker="x")
        ax.scatter(p_gen_end[:, 0].cpu(), p_gen_end[:, 2].cpu(),
                   alpha=0.5, s=8, label="gen end-EE", color="C1")
        ax.set_xlabel("p_x"); ax.set_ylabel("p_z")
        ax.set_title(f"end-EE  pos_err={pos_err_gen.mean():.4f}\nsucc={success_gen:.2f}")
        ax.legend(fontsize=7); ax.grid(alpha=0.3)
        ax.set_aspect("equal")

    fig.suptitle(f"Ours-UNet on Franka 7-DoF (goal-cond, full pipeline)  ({args.metric})")
    fig.tight_layout()
    fig.savefig(out_dir / f"franka_traj_unet_{args.metric}.png", dpi=120)
    plt.close(fig)
    ...
    ax.set_title(f"Ours-UNet training loss ({args.metric})")
    ax.grid(alpha=0.3)
    fig2.tight_layout()
    fig2.savefig(out_dir / f"franka_traj_unet_loss_{args.metric}.png", dpi=120)
    plt.close(fig2)
